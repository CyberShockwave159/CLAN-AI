...
- **Server-side roleplay memory (A-PROX)** — Settings → RAG Memory → *Use A-PROX server memory* hands roleplay memory to the server. Completed turns are pushed into a per-character, per-thread A-PROX collection (`clan_<characterId>_<threadId>`) via `POST /rag/ingest` (a direct store write — no LLM round-trip), and each turn is retrieved by A-PROX's `a-prox-rag` route, which injects the recalled context server-side. Existing threads are backfilled once. The two backends are **mutually exclusive** — the local RAG pipeline is bypassed entirely rather than run alongside — and locally stored embeddings are left untouched, so switching back is lossless. The RAG Top-K and minimum-relevance sliders drive the server query in this mode too. Only offered when the connected server reports the A-PROX capability.
- **Roleplay tool restriction** — Every message of a roleplay turn is tagged `"roleplay": true`, which makes A-PROX restrict the armed tool set to memory retrieval and image generation. A roleplay turn can never reach web search, page fetching, the clock, or file writing, so a reply can't break character to look something up.
- **Scene image generation (A-PROX, roleplay only)** — A `Generate image` action in the assistant message's action row turns the last few exchanges into a picture of the current scene. The result becomes a **new sibling variant of the message**: the original dialogue is kept verbatim and the image is attached, so the existing `‹ 1 / 2 ›` arrows flip between "text only" and "text + picture" exactly as they do for regenerations. The picture exists for the reader only — assistant images are never re-sent to the model, so the next reply is driven purely by the text.
- **Character consistency for generated images** — three layers, because a character who looks different in every picture breaks immersion faster than anything else:
  - *Reference conditioning* — the character's identity reference is attached to the request as an `image_url` part, which makes A-PROX use its image-to-image path. That path is Qwen-Image-Edit-style reference conditioning (the reference conditions the text encoder, it does not seed a latent), which is what preserves a face. Priority: an approved portrait, then the character card's avatar. With neither, the app offers to generate a portrait once.
  - *Appearance sheet* — a canonical physical description per character (editable in Character Edit, LLM-drafted once from the character card with a review step), pinned into every image prompt. Appearance only: wardrobe and setting are deliberately excluded, since they should change with the scene.
  - *Visual style* — `Anime`, `Semi-realistic`, or `Photo-realistic`, applied both in the prompt and as an A-PROX `image_style` field resolved at the workflow level, so a prompt-rewriting model cannot dilute it. Defaults to unset (the model's own choice); a *Detect* button preselects a style from the avatar, which you can override.
  - Long-pressing a generated image refines from *that* image instead of the character's portrait — the way to recover from a bad generation. The toolbar button always restarts from the portrait.
- **Roleplay is text-only** — the composer hides the image-attach button, and a user photo can never enter a roleplay turn.
...
- **File & image artifacts** — When the server (A-PROX `/flags`) emits `delta.image_url` / `delta.file_url` in the SSE stream (or `message.image_url`/`message.file_url` on non-streaming responses), the client resolves the artifact bytes once into the message attachment store and persists the local path + original filename + MIME in SQLite (schema v14 `file_path`/`file_name`/`file_mime` columns). Both served `http(s)://…` URLs and base64 `data:` URLs (A-PROX `[image_generation]/[file_generation].inline_data_url`) are accepted — inline payloads are decoded client-side instead of fetched. Assistant images render inline; the artifact URL is always surfaced as a small tappable link under the image (persisted as schema v15 `image_url`) so a tap-through path survives failed renders/downloads; generated files appear as distinct, tappable document objects at the end of the response (`.txt` shows as a generic text document, `.json` as a data object, `.csv` as a spreadsheet, `.pdf` as a PDF, ...); tapping one saves/exports it through the same `FileSaver` path used for chat export. File URLs the model embeds in its markdown (extension in `TextSanitizer.artifactExtensions`) get the same object treatment, deduplicated against stored A-PROX artifacts.
- SQLite local persistence with full thread/message history (schema v16)
- Automatic server health polling with fallback endpoints (`/health` → `/props` → `/v1/models`); A-PROX reports its capabilities (`rag`, `image`, `file`) there, which is how A-PROX-only features stay hidden against other backends
...
flutter test           # runs all 35 test files (602 total tests)
flutter run            # launch app
```

### Testing

35 test files, 602 total tests. All tests use fake repositories (no real SQLite or network). ViewModels expose private state via setters for test injection.
